...
#Securely load API key for Gemini with .env
...
class GmailAuthPanel(QDialog):
    ...
    def apply_styles(self):
        ...
        """)
    #OAuth flow –runs in background thread
...
class ApplicationEntryPanel(QWidget):
    ...
    def _handle_gmail_auth_success(self, app_data):
        ...
        # Set the flag so it doesn't ask again
        self.is_gmail_connected = True
        self.gmail_connected.emit(True) 
        app_data['auto_monitor'] = True
        #emit the signal to save the application data and clear the form
    ...
    def save_application_prompt_gmail(self):
        ...
        if app_data['auto_monitor']:
            if not self.is_gmail_connected:
                    #Gmail is not connected, so must authenticate.
                ...
                gmail_auth_dialog.exec()
                    ...
                    #Gmail is already connected, so can save immediately.
...
class ApplicationDashboardPanel(QWidget):
    ...
    def _ask_ai_question(self, question):
        ...
        panel_instance = self  #Get a safe reference
    ...
    def _create_application_card(self, app_data):
        ...
        # store its ID instead of the widget 
    ...
    def handle_app_update(self, action, data):
        ...
        if action == "delete":
            ...
            if app_found:
                ...
                self.save_applications()
                self.update_app_dashboard.emit(self.applications)  #refresh fr fr
            else:
